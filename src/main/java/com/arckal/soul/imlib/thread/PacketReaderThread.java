...
import com.arckal.soul.imlib.CommandGroup;
...
public class PacketReaderThread extends Observable implements Runnable {
    ...
    public PacketReaderThread(InputStream in, SoulChatClient client, ChatService chatService ){
        this.inputStream = in;
        this.messageDispatcher = new MessageDispatcher(client,chatService);
    }
            ...
            if (a(bArr)){// 数据包是0102开头
                ...
                try {
                    if (bArr[4] != (byte) 0) {
                        ...
                        // 解析
                        messageDispatcher.handle(CommandGroup.parseFrom(data));
                    } else {
                        // 解析
                        messageDispatcher.handle(CommandGroup.parseFrom(obj));
                    }
                } catch (Exception e) {
                    e.printStackTrace();
